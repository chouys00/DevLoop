...
**先做需求想像，再開任務**：站在實際使用者的角度，設想 2–3 個具體
使用情境（他會怎麼用這個工具、哪裡卡、用完還會想要什麼），從情境
推導需求，再收斂成任務。禁止只從程式碼內部視角（重構、補測試）出發。

提出 **0–5 個**新任務寫入 `TASKS.md`，狀態 `proposed`、來源 `ai`。
**類別配比：至少一半須為使用者可感知的新功能或 UI/UX 改善**
（新子指令、新旗標、輸出排版與可讀性、互動與提示訊息品質）；
技術品質類（重構、錯誤處理、測試覆蓋）最多佔一半。

範圍以 `GOAL.md` 的**用途**為準：符合工具用途的新功能都可發想，
不受既有指令集限制；但仍受「限制」與「絕不做」約束，涉及資料格式
或對外介面變更者照常走步驟 3 閘門。沒有具實質價值的任務就寫 0 個，
不硬湊。0 個 = 「無可發想」→ 觸發停止條件 4。
