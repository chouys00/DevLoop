...
- Windows 測試清理暫存目錄必帶 `fs.rmSync(dir, { recursive: true, maxRetries: 10, retryDelay: 100 })`：防毒/索引器短暫鎖檔會造成低機率 ENOTEMPTY flake。此類 flake 單檔迴圈難重現，要「整套並行 + 多次迴圈」施壓才穩定重現；驗收單跑一次測試不夠，可疑時連跑多次。
