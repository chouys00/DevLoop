...
7. **發想**（若 `TASKS.md` 見底）：檢視現有成果，提出 0–5 個新任務進 `proposed`（沒有具實質價值的任務就 0 個，不硬湊；0 個視為目標完成訊號）
...
- `TASKS.md` 內未解決的 `blocked` 任務累計達 2 個 → 停下回報（代表環境或方向有系統性問題）。
- 來源 `ai` 的任務累計達 10 個 → 停下請使用者檢視方向（防發想失控）。
